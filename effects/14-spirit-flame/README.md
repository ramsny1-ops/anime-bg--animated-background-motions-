...
Stylized spirit fire rises in layered wisps with turbulence, glow, ember particles, and wind interaction.

Part of **anime-bg**, a dependency-free collection of flexible anime-inspired Canvas backgrounds.

## Overview

Spirit Flame is designed as a production-ready visual background rather than a one-off animation snippet. It keeps rendering, configuration, UI controls, diagnostics, persistence, and export behavior separated enough to customize safely.
...
Then open `effects/14-spirit-flame/index.html` through the local server.
