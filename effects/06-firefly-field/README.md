...
> Organic glowing wander. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Independent drift, pulse phases, and radial gradients create a quiet firefly field that reacts gently to the pointer.
...
- effect-specific controls: `glow, wander, pulse`;
...
This effect additionally exposes `glow, wander, pulse`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(n), with gradient work per particle**.
