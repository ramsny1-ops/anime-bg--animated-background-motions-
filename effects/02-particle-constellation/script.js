...
const extra = {
  linkDistance: document.querySelector("#linkDistance"),
  linkDistanceValue: document.querySelector("#linkDistanceValue"),
  twinkle: document.querySelector("#twinkle"),
  twinkleValue: document.querySelector("#twinkleValue")
};
...
class Star {
  ...
  update(dt) {
    const drift = config.speed * dt * 60;
    this.x += this.driftX * drift;
    this.y += this.driftY * drift;
    this.phase += dt * random(0.5, 1.1);
    ...
    if (config.pointerEnabled && pointer.active) {
      ...
      if (d < 180) {
        const pull = (1 - d / 180) * 0.08 * config.intensity;
        this.x += dx * pull * dt * 60;
        this.y += dy * pull * dt * 60;
      }
    }
  }

  draw() {
    const twinkle = 1 + Math.sin(this.phase * 2.2) * 0.45 * config.twinkle;
    const radius = Math.max(0.45, config.particleSize * this.depth * twinkle);
    const alpha = clamp(0.35 + this.depth * 0.65, 0, 1);
    ...
    if (radius > 2.1) {
      ...
      ctx.lineTo(this.x + radius * 3, this.y);
      ctx.moveTo(this.x, this.y - radius * 3);
      ctx.lineTo(this.x, this.y + radius * 3);
      ctx.strokeStyle = rgba(config.particleColor, alpha * 0.18);
      ctx.lineWidth = 0.7;
      ctx.stroke();
    }
  }
...
function syncStarCount() {
  while (stars.length < config.particleCount) stars.push(new Star());
  if (stars.length > config.particleCount) stars.length = config.particleCount;
}
...
function drawConstellations() {
  ...
  for (let i = 0; i < stars.length; i += 1) {
    ...
    for (let j = i + 1; j < stars.length && links < 4; j += 1) {
      const b = stars[j];
      const dx = a.x - b.x;
      const dy = a.y - b.y;
      ...
      links += 1;
      const alpha = (1 - Math.sqrt(sq) / max) * 0.24 * config.intensity;
      ctx.beginPath();
      ctx.moveTo(a.x, a.y);
      ctx.lineTo(b.x, b.y);
...
function loop(time) {
  ...
  updateStats(stars.length);
  frameId = requestAnimationFrame(loop);
}

function syncExtra() {
  extra.linkDistance.value = config.linkDistance;
  extra.linkDistanceValue.value = `${Math.round(config.linkDistance)} px`;
  extra.twinkle.value = config.twinkle;
  extra.twinkleValue.value = config.twinkle.toFixed(2);
}

function readExtra() {
  config.linkDistance = Number(extra.linkDistance.value);
  config.twinkle = Number(extra.twinkle.value);
  syncExtra();
}
...
for (const input of [...Object.values(controls), extra.linkDistance, extra.twinkle]) {
...
ui.random.addEventListener("click", () => {
  ...
  config.twinkle = random(0.2, 1.8);
  config.linkDistance = randomInt(75, 180);
  applyControls(config);
  syncExtra();
  rebuild();
});
