...
> Animated glyph streams. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Column-based drops render changing glyphs with translucent frame clearing. Pointer proximity increases local brightness.
...
- effect-specific controls: `columns, trail, glyphRate`;
...
This effect additionally exposes `columns, trail, glyphRate`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(columns × visible glyphs)**.
