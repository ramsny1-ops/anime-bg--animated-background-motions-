...
> Sine and cosine motion field. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Rows of particles are positioned from mathematical wave functions. Amplitude, frequency, phase, row count, and pointer displacement create the visual structure.
...
- effect-specific controls: `amplitude, frequency, rows`;
...
This effect additionally exposes `amplitude, frequency, rows`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(n) position calculations and draws**.
