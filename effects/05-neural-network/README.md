...
> Layered graph with animated signal pulses. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Procedurally laid-out nodes form a layered network. Connections carry pulse objects that interpolate from source to destination.
...
- effect-specific controls: `layers, pulseRate, connectionOpacity`;
...
This effect additionally exposes `layers, pulseRate, connectionOpacity`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(nodes + connections + pulses)**.
