...
> Depth, twinkle, and proximity links. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

A layered star field uses independent phases and depth values to produce twinkle, while a bounded number of nearby links forms transient constellations.
...
- effect-specific controls: `linkDistance, twinkle`;
...
This effect additionally exposes `linkDistance, twinkle`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
{
  "project": "anime-bg",
  "effect": "02-particle-constellation",
  "schemaVersion": 1,
...
Default computational profile: **O(n²) proximity checks with link limiting**.
