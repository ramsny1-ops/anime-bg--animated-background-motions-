...
const extra = {
  ...
  trailLengthValue: document.querySelector("#trailLengthValue"),
  turnRate: document.querySelector("#turnRate"),
  turnRateValue: document.querySelector("#turnRateValue")
};
...
class TrailParticle {
  ...
  update(dt, time) {
    ...
    const frame = dt * 60;
    const noiseLike = Math.sin(time * 0.001 + this.seed + this.x * 0.006) +
      Math.cos(time * 0.0007 + this.seed * 1.4 + this.y * 0.005);

    this.angle += noiseLike * 0.012 * config.turnRate * config.intensity * frame;

    if (config.pointerEnabled && pointer.active) {
      const target = Math.atan2(pointer.y - this.y, pointer.x - this.x);
      let delta = target - this.angle;
      delta = Math.atan2(Math.sin(delta), Math.cos(delta));
      const direction = pointer.down ? -1 : 1;
      this.angle += delta * 0.01 * config.intensity * direction * frame;
    }

    const velocity = this.velocity * config.speed * frame;
  ...
  draw(index) {
    const mix = index / Math.max(1, particles.length - 1);
    ctx.beginPath();
    ctx.moveTo(this.prevX, this.prevY);
    ctx.lineTo(this.x, this.y);
    ctx.strokeStyle = rgba(config.accentColor, 0.2 + mix * 0.65);
    ctx.lineWidth = Math.max(0.5, config.particleSize * (0.5 + mix));
    ctx.stroke();

    ctx.beginPath();
    ctx.arc(this.x, this.y, Math.max(0.5, config.particleSize * 0.55), 0, Math.PI * 2);
...
function syncCount() {
  while (particles.length < config.particleCount) particles.push(new TrailParticle());
...
function loop(time) {
  ...
  ctx.fillStyle = rgba(config.backgroundColor, config.trailLength);
...
function syncExtra() {
  extra.trailLength.value = config.trailLength;
  extra.trailLengthValue.value = config.trailLength.toFixed(2);
  extra.turnRate.value = config.turnRate;
  extra.turnRateValue.value = config.turnRate.toFixed(2);
}

function readExtra() {
  config.trailLength = Number(extra.trailLength.value);
  config.turnRate = Number(extra.turnRate.value);
  syncExtra();
}
...
for (const input of [...Object.values(controls), extra.trailLength, extra.turnRate]) {
...
ui.random.addEventListener("click", () => {
  ...
  config.speed = random(0.5, 2.5);
  config.turnRate = random(0.2, 3.8);
  config.trailLength = random(0.04, 0.25);
  config.particleCount = randomInt(40, 180);
  applyControls(config);
  syncExtra();
  particles = [];
  syncCount();
