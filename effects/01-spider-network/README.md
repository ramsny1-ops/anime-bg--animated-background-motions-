...
> Distance-based particle graph. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Particles drift through the canvas and create temporary edges when neighbors enter a configurable radius. Pointer interaction can repel or attract the network.
...
- effect-specific controls: `connectionDistance, lineWidth, mouseRadius`;
...
This effect additionally exposes `connectionDistance, lineWidth, mouseRadius`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(n²) pair checks in the simple implementation**.
