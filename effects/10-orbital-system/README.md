...
> Multi-attractor artistic gravity. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Particles begin with tangential velocity and accumulate forces from multiple attractors, creating unstable but visually useful orbital paths.
...
- effect-specific controls: `gravity, attractors, trail`;
...
This effect additionally exposes `gravity, attractors, trail`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(particles × attractors)**.
