...
> Interactive vector force field. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Particles carry velocity and mass, then respond to a distance-falloff force centered on the pointer. Holding the pointer reverses the force.
...
- effect-specific controls: `force, radius, friction`;
...
This effect additionally exposes `force, radius, friction`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(n) particle updates**.
