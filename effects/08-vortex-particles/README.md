...
> Radial plus tangential force simulation. A standalone anime-bg effect built with semantic HTML, responsive CSS, Canvas 2D, and dependency-free modern JavaScript.

## Overview

Particles combine center pull with a perpendicular spin force, producing spiral motion around the pointer or canvas center.
...
- effect-specific controls: `pull, spin, trail`;
...
This effect additionally exposes `pull, spin, trail`. Read the corresponding values in `DEFAULTS` and search for each property in `script.js` to see exactly where it enters the math or drawing logic.
...
Default computational profile: **O(n) force updates**.
